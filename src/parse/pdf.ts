...
import { groupLines, type Page, type RawItem } from './layout';
...
export async function readPdfPages(data: ArrayBuffer, password?: string): Promise<Page[]> {
  return (await readPdfItems(data, password)).map((items) => groupLines(items));
}

/** Raw positioned text items per page, so a statement can be re-read with different settings. */
export async function readPdfItems(data: ArrayBuffer, password?: string): Promise<RawItem[][]> {
